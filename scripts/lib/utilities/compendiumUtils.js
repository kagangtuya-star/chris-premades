...
function _normalizeItemName(name) {
    ...
    let normalized = name.replace(/[\u4e00-\u9fff]/g, "");
...
async function getCPRAutomation(item, { identifier, rules = "legacy", type = "character" } = {}) {
    ...
    let name =
        custom.getMacro(itemIdentifier, rules)?.name ?? CONFIG.chrisPremades.renamedItems[item.name] ?? item.name;
    let folderId;
    ...
    const normalizedItemName = _normalizeItemName(item.name);
    for (let key of keys) {
        let found;
        if (cprIdentifier) {
            found = await getItemFromCompendium(key, cprIdentifier, {
                ignoreNotFound: true,
                folderId: folderId,
                byIdentifier: true,
            });
        } else {
            found = await getItemFromCompendium(key, normalizedItemName, { ignoreNotFound: true, folderId: folderId });
        }
...
async function getGPSAutomation(item, { identifier, rules = "legacy", type = "character" } = {}) {
    ...
    if (type === "character" || item.type === "spell") {
        switch (item.type) {
            case "spell":
                found = gambitPremades.gambitItems.find(
                    (i) => _normalizeItemName(i.name) === normalizedItemName && i.type === "spell" && i.rules === rules
                );
                break;
            case "weapon":
            case "equipment":
            ...
            case "loot":
                found = gambitPremades.gambitItems.find(
                    (i) =>
                        _normalizeItemName(i.name) === normalizedItemName &&
                        constants.itemTypes.includes(i.type) &&
                        i.rules === rules
                );
                break;
            case "feat":
                found = gambitPremades.gambitItems.find(
                    (i) => _normalizeItemName(i.name) === normalizedItemName && i.type === "feat" && i.rules === rules
                );
                break;
        }
    } else if (type === "npc") {
        let monster = identifier ?? item.actor.prototypeToken.name;
        found = gambitPremades.gambitMonsters.find(
            (i) =>
                i.monster === monster &&
                _normalizeItemName(item.name) === _normalizeItemName(i.name) &&
                i.rules === rules
        );
    }
    if (!found) return;
...
async function getMISCAutomation(item, { identifier, rules = "legacy", type = "character" } = {}) {
    ...
    if (type === "character" || item.type === "spell") {
        switch (item.type) {
            case "spell":
                found = miscPremades.miscItems.find(
                    (i) => _normalizeItemName(i.name) === normalizedItemName && i.type === "spell" && i.rules === rules
                );
                break;
            case "weapon":
        ...
        let monster = identifier ?? item.actor.prototypeToken.name;
        found = miscPremades.miscMonsters.find(
            (i) =>
                i.monster === monster &&
                _normalizeItemName(item.name) === _normalizeItemName(i.name) &&
                i.rules === rules
...
async function getAllAutomations(item, options = {}) {
    ...
    options.type = type;
    await Promise.all(
        Object.entries(setting).map(async (i) => {
            let found;
            let source;
            let version;
            switch (i[0]) {
                default:
                    found = await getItemFromCompendium(i[0], item.name, {
                        ignoreNotFound: true,
                        matchType: item.type,
                        rules: options?.rules,
                    });
                ...
                case "gambits-premades":
                    ...
                    if (found) {
                        if (type === "npc") {
                            version = gambitPremades.gambitMonsters.find(
                                (i) =>
                                    _normalizeItemName(i.name) === _normalizeItemName(item.name) &&
                                    i.monster === monster
                            )?.version;
                        } else {
                            version = gambitPremades.gambitItems.find(
                                (i) => _normalizeItemName(i.name) === _normalizeItemName(item.name)
                            )?.version;
                        }
                    }
                    break;
                case "midi-item-showcase-community":
                    ...
                    version = miscPremades.miscItems.find(
                        (i) => _normalizeItemName(i.name) === _normalizeItemName(item.name)
                    )?.version;
                    if (found) {
                        if (type === "npc") {
                            version = miscPremades.miscMonsters.find(
                                (i) =>
                                    _normalizeItemName(i.name) === _normalizeItemName(item.name) &&
                                    i.monster === monster
                            )?.version;
            ...
            if (found)
                items.push({
                    ...
                    priority: i[1],
                    source: source,
                    version: version ?? itemUtils.getVersion(found),
                });
        })
    );
...
async function getItemFromCompendium(
    key,
    name,
    {
        ignoreNotFound,
    ...
    let packIndex = await pack.getIndex({
        fields: ["name", "type", "folder", "system.source.rules", "flags.chris-premades.info.identifier"],
    });
    let match;
    const normalizedItemName = _normalizeItemName(name);
    if (!byIdentifier) {
        match = packIndex.find(
            (item) =>
                (item.flags["chris-premades"]?.info?.aliases ?? [])
                    .concat(item.name)
                    .map(_normalizeItemName)
                    .includes(normalizedItemName) &&
                (!folderId || (folderId && item.folder === folderId)) &&
                (!matchType || item.type === matchType) &&
                (!rules || item.system.source.rules === (rules === "modern" ? "2024" : "2014"))
        );
    } else {
        match = packIndex.find(
            (item) =>
                item.flags["chris-premades"]?.info?.identifier === name &&
                (!folderId || (folderId && item.folder === folderId)) &&
                (!matchType || item.type === matchType) &&
                (!rules || item.system.source.rules === (rules === "modern" ? "2024" : "2014"))
        );
    ...
    if (match) {
        ...
        if (object) {
            ...
            if (flatAttack) {
                let activityIds = Object.entries(activities)
                    .filter((i) => i[1].type === "attack")
                    .map((i) => i[0]);
            ...
            if (castDataWorkflow) {
                ...
                genericUtils.setProperty(
                    documentData,
                    "flags.chris-premades.castData.school",
                    castDataWorkflow.item.system.school
                );
            }
            return documentData;
        } else {
...
async function getActorFromCompendium(
    key,
    name,
    { ignoreNotFound, folderId, object = false, translate, identifier } = {}
) {
    let pack = game.packs.get(key);
    if (!pack) {
        if (!ignoreNotFound) errors.missingPack();
    ...
    let match = packIndex.find(
        (item) =>
            _normalizeItemName(item.name) === _normalizeItemName(name) &&
            (!folderId || (folderId && item.folder === folderId))
    );
    if (match) {
        let document = await pack.getDocument(match._id);
...
async function getFilteredActorDocumentsFromCompendium(
    key,
    { maxCR, actorTypes, creatureTypes, creatureSubtypes, specificNames } = {}
) {
    let pack = game.packs.get(key);
    let packIndex = await pack.getIndex({
        fields: ["name", "type", "img", "system.details.cr", "system.details.type", "system.attributes.movement"],
    });
    let filteredIndex = packIndex.filter(
        (i) =>
            ...
            (!specificNames?.length || specificNames.map(_normalizeItemName).includes(_normalizeItemName(i.name)))
    );
    filteredIndex = game.dnd5e.moduleArt.apply(filteredIndex);
    filteredIndex = filteredIndex.map((i) =>
        foundry.utils.mergeObject(i, { img: "icons/svg/mystery-man.svg" }, { overwrite: !i.img })
    );
...
async function getFilteredItemDocumentsFromCompendium(key, { specificNames, types, typeValues, badProperties } = {}) {
    ...
    filteredIndex = game.dnd5e.moduleArt.apply(filteredIndex);
    filteredIndex = filteredIndex.map((i) =>
        foundry.utils.mergeObject(i, { img: "systems/dnd5e/icons/svg/items/weapon.svg" }, { overwrite: !i.img })
    );
    return filteredIndex;
}
...
async function getAppliedOrPreferredAutomation(item, options) {
    ...
    if (source) {
        switch (source) {
            ...
            default: {
                let document = await getItemFromCompendium(source, item.name, {
                    ignoreNotFound: true,
                    rules: options?.rules,
                });
